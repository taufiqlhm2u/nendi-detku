...
new class extends Component {
    ...
    {
        return Income::whereYear('date', $this->filterYear)->whereMonth('date', $this->filterMonth)->sum('amount');
    }

    #[Computed]
    public function expenseTotal(): float
    {
        return Expense::whereYear('date', $this->filterYear)->whereMonth('date', $this->filterMonth)->sum('amount');
    }
    ...
    public function groupedTransactions(): array
    {
        ...
        if ($this->filter !== 'pengeluaran') {
            Income::with('user')->when($this->search, function($a) {
                $a->whereHas('user', function($b) {
        ...
        if ($this->filter !== 'pemasukan') {
            Expense::with('user')->when($this->search, function($c) {
                $c->whereHas('user', function($d) {
...
<div>
    ...
    <main class="px-4 pt-4 space-y-5 max-w-xl mx-auto relative">
        ...
        <h1 class="text-xl font-bold tracking-tight">Semua Riwayat</h1>
        {{-- Filter Controls --}}
        <div class="space-y-3 page-fade" style="--delay: 0s">

            {{-- Month & Year Selectors --}}
            <div class="flex justify-between items-center gap-2">
                <label class="input w-50">
                    ...
                    <input type="search" placeholder="Cari user..." wire:model.live.debounce.300ms="search" />
                </label>
                <div>
                    ...
                    @foreach ($group['items'] as $tx)
                        <div
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-[11px] font-medium text-[#191c21]/60 truncate">
