<nav class="fixed bottom-0 left-0 right-0 bg-[#f9f9ff] border-t border-base-300 shadow-lg z-50">
    <div class="flex items-center justify-around px-4 py-2 max-w-2xl mx-auto">
        ...
        <a href="{{ route('admin.user.index') }}"
            class="flex flex-col items-center justify-center gap-1 px-3 py-1 transition-colors {{ request()->routeIs('admin.user*') ? 'text-primary' : 'text-base-content/50 hover:text-primary' }}">
            <span class="material-symbols-outlined text-[26px]"
                style="{{ request()->routeIs('admin.user') ? 'font-variation-settings: \'FILL\' 1;' : '' }}">groups</span>
            <span class="text-xs {{ request()->routeIs('admin.user*') ? 'font-semibold' : 'font-medium' }}">User</span>
        </a>
