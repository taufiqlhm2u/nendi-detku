...
new class extends Component {
    ...
    {
        $i = $this->income;
        $this->amount = old('amount', $i->amount);
        $this->type = old('type', $i->type);
        $this->date = old('date', $i->date->translatedFormat('Y-m-d'));
        $this->note = old('note', $i->note);
    }
};
?>
    ...
    <header class="sticky top-0 z-50 bg-base-100 border-b border-base-300/40 backdrop-blur-md">
        <div class="max-w-xl mx-auto flex items-center justify-between px-4 py-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('incomes.show', $this->income->id) }}" class="btn btn-ghost btn-sm btn-circle">
                    <span class="material-symbols-outlined text-[24px] text-default">arrow_back_ios_new</span>
                </a>
                <h1 class="text-lg font-bold text-default">Edit Pemasukan</h1>
            </div>
        </div>
    </header>
    ...
    <main class="px-4 pt-6 max-w-xl mx-auto space-y-6">
        ...
        <div class="card bg-white border border-base-300/40 shadow-sm">
            <div class="card-body gap-5">
                ...
                <form id="incomeForm" method="post" action="{{ route('incomes.update', $this->income->id) }}" class="flex flex-col gap-5"
                    ...
                    <div class="form-control gap-1" x-data="{
                        raw: '{{ old('amount', $this->income->amount) }}',
                    ...
                    <div class="form-control gap-1" x-data="{
                        ...
                        errorMsg: '',
                        showMenu: false,
                        existingPhotoUrl: '{{ $this->income->image ? asset('storage/' . $this->income->image) : '' }}',
